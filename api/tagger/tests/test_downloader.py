...
def test_url_data():
    assert requests.get(URL_DATA).status_code == 200


def test_get_filename_from_cd():
    cd = 'attachment; filename="data.tar.gz"; filename*=UTF-8''data.tar.gz'
    assert Downloader.get_filename_from_cd(cd) == 'data.tar.gz'
...
def test_downloader(mock_tarfile, mock_get, _tmp_dir):
    ...
    model_tarfile = tarfile.open(os.path.join(
        _tmp_dir.strpath, 'model.tar.gz'), 'r:gz')
